...
@app.route('/sentry')
def sentry():
    sentryinfo=session.query(sentry.id,sentry.energy, sentry.ip, sentry.ts, sentry.date, sentry.sigma_vi, sentry.ps, sentry.des)
    sentry_df=pd.DataFrame(sentryinfo, columns=['id','energy','ip', 'ts', 'date', 'sigma_vi', 'ps', 'des'])
    sentrydata=[]
    for id, energy, ip, ts, date, sigma_vi, ps, des in sentryinfo:
        cleansentry={}
        cleansentry['id']= id
